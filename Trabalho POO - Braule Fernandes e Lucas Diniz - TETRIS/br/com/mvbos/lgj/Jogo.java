...
public class Jogo extends JFrame {
	...
	private static final int JANELA_LARGURA = 1000; //AUMENTANDO O TAMANHO DA JANELA PARA OS PAINEIS
	...
	private void setaTecla(int tecla, boolean pressionada) {
		...
		case KeyEvent.VK_SPACE:
			...
			controleTecla[Tecla.SPACE.ordinal()] = pressionada; //OUTRA TECLA ADICIONADA!!
			break;
	...
	public void iniciarJogo() {
		...
		while (true) {
			if (System.currentTimeMillis() >= prxAtualizacao) { //System.currentTimeMillis() -> Serve para medir intervalos de tempos
				...
				if (controleTecla[Tecla.BA.ordinal()]) {
					...
					if (cenario instanceof InicioCenario) {
						cenario.descarregar();
						cenario = null;
						cenario = new JogoCenario(tela.getWidth()-500, tela.getHeight()); //PARA MANTER O JOGO SIMETRICO
				...
				if (cenario == null) {
					...
					if (Jogo.pausado) {
						g2d.setColor(Color.WHITE);
						g2d.drawString("Jogo Pausado!", 87, 142); //JOGO PAUSADO
					}
				}
	...
	public static void main(String[] args) { //inicio do jogo
		Jogo jogo = new Jogo();
		jogo.carregarJogo();
		jogo.iniciarJogo();
	}
